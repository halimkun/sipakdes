  ...
  <div class="col-12 col-md-9">
    <div class="card card-outline card-success">
      <div class="card-header">
        <h4 class="card-title">
          <i class="fa fa-fingerprint mr-1"></i> Pengajuan SKCK
        </h4>
      </div>
      <div class="card-body p-3">
        <div class="table-responsive mb-0">
          <table class="table table-striped" id="table-pengantar">
            ...
            <tbody>
              <?php foreach ($data as $key => $skck) : ?>
                <tr>
                  <td><?= $key + 1 ?></td>
                  <!-- <td></td> -->
                  <td>
                    <p class="p-0 m-0"><?= $skck->nama ?></p>
                  ...
                  <td>
                    <?php if ($skck->status == 'pending') : ?>
                      <span class="badge badge-info">Pending</span>
                    <?php elseif ($skck->status == 'ditolak') : ?>
                      <span class="badge badge-danger">Ditolak</span>
                    <?php elseif ($skck->status == 'selesai') : ?>
                      <span class="badge badge-success">Selesai</span>
                    <?php elseif ($skck->status == 'batal') : ?>
                    ...
                    <?php if (!in_groups('warga')) : ?>
                      ...
                      <div class="btn-group">
                        ...
                        <div class="dropdown-menu">
                          <a class="dropdown-item" href="/surat/pengantar/skck/<?= $skck->pengantar_id ?>/print"><i class="fa fa-print mr-1"></i> Cetak</a>
                          <div class="dropdown-divider"></div>
                          <a class="dropdown-item btn-update-status" href="#!" data-id="<?= $skck->pengantar_id ?>" data-nama="<?= $skck->nama ?>" data-tempat-lahir="<?= $skck->tempat_lahir ?>"><i class="fa fa-pen mr-1"></i> Update Status</a>
                        </div>
                      </div>
                    <?php else : ?>
                      <div class="btn-group btn-group-sm" role="group" aria-label="Aksi Penduduk">
                        <?php if (!in_array($skck->status, ['pending'])) : ?>
  ...
  <div class="col-12 col-md-3">
    <div class="card card-outline card-info">
      ...
      <div class="card-body">
        ...
        <!-- button block -->
        <a href="/surat/pengantar/skck/new" class="btn btn-block btn-info">
...
<?php if (!in_groups(['warga'])) : ?>
  <script>
    ...
    $('.btn-update-status').on('click', function(e) {
      ...
      $('#modal-update-status form').attr('action', `/surat/pengantar/skck/${id}/update-status`);
    });
  </script>
<?php endif ?>
<?= $this->endSection(); ?>